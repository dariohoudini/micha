...
export default function ProfilePage() {
  ...
  useEffect(() => {
    Promise.allSettled([
      client.get('/api/v1/orders/my/').then(r => r.data.count || r.data.results?.length || 0),
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
              ...
              <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, fontWeight: 700, color: '#0A0A0A' }}>{initial}</span>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: '#FFFFFF', marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {user?.username || user?.email?.split('@')[0]}
              </h2>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#9A9A9A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {user?.email}
              </p>
            </div>
            <button onClick={() => navigate('/profile/edit')}
              style={{ width: 36, height: 36, borderRadius: 10, background: '#1E1E1E', border: '1px solid #2A2A2A', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
        ...
        <div style={{ padding: '8px 16px 0' }}>
          {MENU_SECTIONS.map(section => (
            <div key={section.title} style={{ marginBottom: 8 }}>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, fontWeight: 600, color: '#9A9A9A', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '16px 4px 8px' }}>
                {section.title}
              </p>
              <div style={{ background: '#141414', borderRadius: 16, border: '1px solid #1E1E1E', overflow: 'hidden' }}>
                {section.items.map((item, i) => (
                  <button key={item.label} onClick={() => navigate(item.path)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'none', border: 'none', cursor: 'pointer', borderBottom: i < section.items.length - 1 ? '1px solid #1E1E1E' : 'none', textAlign: 'left' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: '#1E1E1E', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: '#FFFFFF', flex: 1 }}>{item.label}</span>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2A2A2A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div style={{ marginTop: 8, marginBottom: 24 }}>
            <button onClick={handleLogout}
              style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 16, background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.15)', cursor: 'pointer', textAlign: 'left' }}>
              ...
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: '#dc2626', fontWeight: 500 }}>Terminar sessão</span>
            </button>
          </div>
        </div>
      </div>
