...
export default function SellerProductNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await client.post('/api/v1/products/', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      ...
      if (data && typeof data === 'object') {
        ...
        showToast(Array.isArray(data[firstKey]) ? data[firstKey][0] : data[firstKey] || 'Erro ao publicar produto.', 'error')
      } else {
        showToast('Erro ao publicar produto. Tente novamente.', 'error')
      }
  ...
  return (
    <SellerLayout title="Novo produto">
      {toast && (
        ...
        <div style={{ padding: '0 16px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {step === 1 && <>
            ...
            <div>
              ...
              <p style={{ ...S, fontSize: 11, color: '#555', marginTop: 6 }}>Primeira foto = imagem principal. Recomendado: fundo branco ou neutro.</p>
            </div>
          ...
          {step === 3 && <>
            ...
            <div style={{ background: '#141414', borderRadius: 16, border: '1px solid #C9A84C', padding: 16 }}>
              ...
              {[
                ...
                { l: 'Fotos', v: `${images.length} foto${images.length !== 1 ? 's' : ''}` },
